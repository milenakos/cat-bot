...
async def cataine(message: discord.Interaction):
    ...
    async def pay_cataine(interaction):
        ...
        for i in range(user.bounties):
            if (
                ...
                return
        if user.cataine_price:
            if user[f"cat_{user.cataine_price}"] < user.cataine_amount:
                await interaction.response.send_message("You don't have enough cats!", ephemeral=True)
                return
            user[f"cat_{user.cataine_price}"] -= user.cataine_amount
        ...
        if user.cataine_level != 10:
            ...
            user.hibernation = True
            if user.cataine_level == 1:
                user.cataine_active = int(time.time()) + 3600
        else:
    ...
    async def perk_screen(interaction):
        ...
        if user.perk1 and user.perk2 and user.perk3:
            ...
            perks = [p["uuid"] for p in await get_perks(user.cataine_level, user)]
        ...
        perk_embed.add_item(TextDisplay("-# The cataine timer will not start until you begin your bounties."))
    ...
    async def gen_main():
        ...
        if not level == 0 and not user.hibernation:
            if int(time.time()) - user.cataine_active < 1800:
                desc += f"\n\n**Hurry!** Levels down <t:{user.cataine_active}:R> ({duration}h total)"
            elif user.cataine_active > 0:
                desc += f"\n\nLevels down <t:{user.cataine_active}:R> ({duration}h total)"
