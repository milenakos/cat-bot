...
async def stats_command(
    ...
    async def gen_page(person: discord.User | discord.Member, page: str) -> LayoutView:
        ...
        user_select.callback = change_user
        ...
        page_select.callback = change_page
            ...
            for user_id, cat_types in sorted(reverse_mapping.items(), key=lambda item: len(item[1]), reverse=True):
                ...
                if show_cats != "":
                    if shortened_types:
                        ...
                        show_cats = ": " + show_cats
                starter = str(config.rain_starter[channel.channel_id]).strip()
                if len(starter) > 10 and starter in str(user_id).strip():
                    part_one += "☔ "
...
async def battlepass(message: discord.Interaction):
    ...
    async def gen_main(interaction: discord.Interaction, first: bool = False) -> None:
        ...
        if user.vote_cooldown != 0:
            ...
            if is_weekend:
                description += f" - ~~{user.vote_reward}~~ **{user.vote_reward * 2}** XP"
            else:
                description += f" - {user.vote_reward} XP"
