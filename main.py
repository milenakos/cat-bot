...
async def on_message(message):
    ...
    if text.lower() == "cat":
        ...
        if not is_cat or timestamp > time.time() or (message.author.bot and message.author.id not in WHITELISTED_BOTS):
            ...
            elif randint(0, 15) == 0 and TOP_GG_TOKEN and get_cat(0, message.author.id, "vote_time") + 43200 < time.time():
...
async def dark_market(message):
    ...
    if get_cat(message.guild.id, message.user.id, "cataine_active") == 0 or get_cat(message.guild.id, message.user.id, "cataine_active") < int(time.time()):
        ...
        hoursago = round((get_cat(message.guild.id, message.user.id, "cataine_active") - int(time.time())) / 3600, 2)
        embed = discord.Embed(title="The Dark Market", description=f"you bought from us {hoursago} hours ago. please wait until the next purchase.")
